...
historicalspreads_clean <- read.csv(paste(wd,"historicalspreads_clean_2019incl.csv",sep="/"))
...
# sample_input1[1,] <- c(1, 10, 75, -1, 0, 3600, "KC", "HOU", 9.5)
# sample_input2[1,] <- c(1, 10, 75, 1, 0, 3600, "KC", "HOU", 9.5)
# (predict(modFit_final, sample_input1)+predict(modFit_final, sample_input2))/2



gt <- function(quarter, clockmin, clocksecs){
      if(quarter == 1){
            time <- 2700 + clockmin*60 + clocksecs
      } else if(quarter == 2){
            time <- 1800 + clockmin*60 + clocksecs
      } else if(quarter == 3){
            time <- 900 + clockmin*60 + clocksecs
      } else if(quarter == 4){
            time <- clockmin*60 + clocksecs
      }
      
      time
}

## [RUN THE NEXT LINE TO GET TIMESECS, IF YOU'RE TOO LAZY TO DO THE MATH]
gt(1,15,0)

## [RUN THE NEXT 2 LINES IN-GAME, AFTER EACH PLAY, OR AS DESIRED]
sample_input1[1,] <- c(1, 10, 75, 1, 0, 3600, "SF", "MIN", 7)
predict(modFit_final, sample_input1)

#INPUTS: down, distance, yrdline100, home_team_pos, current home_team_margin, time remaining, home team, away team, pregame spread (vegas)
#home_team_pos is either 1 or -1, depending on whether SF has the ball
